...
function gitanocreatebranch () {
	...
	then 
		...
		if [[ -z $BRANCH_ORIGIN ]]; then
	    	echo " (*) Creating branch $BRANCH (from branch $BRANCH_ORIGIN) locally [git checkout -b $BRANCH $BRANCH_ORIGIN]"
		    git checkout -b $BRANCH $BRANCH_ORIGIN
		    echo " (*) Commiting branch creation [git commit -m @autogeneratedmessage]"
			git commit -m "Creating branch $BRANCH from branch $BRANCH_ORIGIN"
		else
